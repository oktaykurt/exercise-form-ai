...
with device.running():
    for type_, frame in device:
        if type_ is FrameType.Color:
            rgb_image = frame.to_array()
            bgr_image = cv2.cvtColor(rgb_image, cv2.COLOR_BGRA2BGR)
            flipped_image = cv2.flip(bgr_image, 1)
            results = pose.process(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
            ...
            if results.pose_landmarks:
                ...
                pose_coordinates = [current_time, left_hip, left_knee, left_ankle, left_shoulder, left_elbow, left_wrist, right_hip, right_knee, right_ankle, right_shoulder, right_elbow, right_wrist]
